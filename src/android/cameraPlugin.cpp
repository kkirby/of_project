...
CameraPlugin::CameraPlugin() : JavaClass("com/someplace/asdf/CameraPlugin"), recording(false) {}
...
void CameraPlugin::setGrabber(ofxAndroidVideoGrabber* grabber){
	...
	callVoidMethod("setVideoGrabber","Lcc/openframeworks/OFAndroidVideoGrabber;",grabber->data->javaVideoGrabber);
}

void CameraPlugin::startRecording(){
	if(recording)return;
	ofLogVerbose("CameraPlugin") << "Start Recording";
	callVoidMethod("startRecording","");
	recording = true;
}

void CameraPlugin::stopRecording(){
	if(!recording)return;
